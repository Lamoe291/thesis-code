...
import torch
from prime_model import Trainer
...
parser.add_argument('--modelname', default='4s_12_layers_pg')
...
weights = []
if args.weighted == '0':
    weights = [1.0 for i in range(num_classes)]
else:
...
if args.action == "train":
    ...
    params = [final_model_name, args.dataset, args.split, args.layers, args.numstages, args.weighted, args.lambda_param, bz, lr, num_epochs]
    ...
    for i in list(args.split):
        ...
        trainer = Trainer(num_stages, num_layers,kernel_size, num_f_maps, features_dim, num_classes, weights_tensor, logfile_path)
        trainer.train(model, batch_gen, num_epochs=num_epochs, batch_size=bz, learning_rate=lr, device=device, lambda_param=float(args.lambda_param), start_training_epoch=start_training_from_epoch, epoch_loss_acc=[epoch_loss, epoch_acc])
...
if args.action == "predict":
    for i in list(args.split):
        ...
        if not os.path.exists(results):
            os.makedirs(results)
        trainer = Trainer(num_stages, num_layers,kernel_size, num_f_maps, features_dim, num_classes, weights_tensor)
